# Saleh Alharbi

### Clinical Informatics & Digital Health

**PharmD · CPHIMS · PMP**

I work at the intersection of **clinical practice, healthcare technology, pharmacy automation, integration, and digital health** — translating real clinical and operational needs into technology that can be configured, implemented, validated, and improved in practice.

My experience spans clinical systems and medication technology, from requirements and workflow design through configuration, integration, testing, go-live, and optimization.

[![Portfolio](https://img.shields.io/badge/Portfolio-View%20my%20work-2f7058?style=for-the-badge)](https://saleh-portfolio-psi.vercel.app)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/phsaloufi)

---

## Healthcare Technology Experience

**Clinical Informatics**  
Cerner Millennium · requirements · workflow design · application configuration · testing · go-live · optimization

**Pharmacy Automation**  
Pyxis · ROWA · Cenker LogisOne · smart infusion pumps · EHR-connected medication workflows

**Integration & Digital Health**  
ERP–EHR integration · NPHIES · digital prescription refill services · interoperability and workflow validation

**Medication Safety**  
Clinical decision support · medication-safety workflows · antimicrobial stewardship · measured clinical outcomes

---

## Selected Impact

| Scope | Evidence |
|---|---|
| **Clinical systems** | Cerner work across **6 hospitals + 23 primary care centers** |
| **Medication safety** | Clinical decision-support work associated with a **20% reduction in medication errors** |
| **Digital health** | Prescription refill platform supporting **1,000+ transactions/day** |
| **Delivery** | Experience from clinical need and requirements through implementation, go-live, and optimization |

---

## AI Projects

### AI Business Craft
**Business Analysis × AI**

An AI-supported business analysis workspace connecting discovery, requirements, quality, traceability, human review, and professional deliverables.

[Product story](https://saleh-portfolio-psi.vercel.app/projects/ai-business-craft) · [GitHub repository](https://github.com/Skharbi/ai-business-craft)

### AI Doc
**Doctor Workflow × Healthcare AI**

Exploring how AI can reduce Doctor documentation effort and return more attention to the patient, with a workflow centered on fast review and minimal clinician interaction.

[Product story](https://saleh-portfolio-psi.vercel.app/projects/ai-doc) · [GitHub repository](https://github.com/Skharbi/Ai_Doc)

### HealthScope
**Healthcare Intelligence × AI Automation**

A role-aware healthcare strategic-intelligence workflow designed to turn verified global and Saudi/GCC signals into traceable, decision-relevant intelligence while suppressing low-value updates.

[Product story](https://saleh-portfolio-psi.vercel.app/projects/healthscope) · [GitHub repository](https://github.com/Skharbi/healthscope)

---

## How I Work

**Understand → Translate → Deliver → Optimize**

I start with the clinical and operational problem, translate it into requirements and system behavior, carry it through configuration/integration and implementation, then use real-world outcomes and workflow feedback to improve it.

---

### Explore my work

**[Portfolio](https://saleh-portfolio-psi.vercel.app)** · **[LinkedIn](https://www.linkedin.com/in/phsaloufi)**
